...
/* Text on its own scrim, sized to the string rather than the whole strip. The
   glyphs carry their own outline, so the panel is only there to lift text off a
   busy picture: translucent where the format allows it, dithered where a pixel
   can only be all or nothing. */
